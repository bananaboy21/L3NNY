...
class cr:
    ...
    @commands.command()
    async def clashrsave(self, ctx, crtag=None):
        """Saves your CR tag to your account."""
        if crtag is None:
            return await ctx.send("Please enter a tag to save. Usage: *crsave [tag]")
        if not self.check_tag(crtag):
            return await ctx.send(" :x: That must be an invalid tag.")   
        ezjson.dump("data/crtags.json", ctx.author.id, crtag)                
        await ctx.send("Success. :ok_hand: Your tag is now saved to your account.")


    @commands.command()
    async def clashrpro(self, ctx, crtag:str):
        """get your clash royale profile"""
        if crtag is None:
    ...
    @commands.command()
    async def clashrclan(self, ctx, clantag:str):
        """Shows info of a clan."""
        try:
